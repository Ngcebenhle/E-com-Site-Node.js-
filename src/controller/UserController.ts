...
static async registerUserViaPhoneNumber(req, res, next) {
  const phone = req.query.phone;
  // const check_user = req.query.register || null
  let user = req.user;
...
static async authonticateUserViaPhoneNumber(req, res, next) {
  const phone = req.query.phone;
  ...
  try {
    ...
    if(!user) throw('Wrong Opt/ Otp is expired')
...
static async updateCustomerprofile(req, res, next) {
  ...
  try {
    ...
    const userdata = await userModel.findById(user.aud);
    ...
    // const refresh_token = await Jwt.jwtSignRefresgToken(payload,user._id, { expiresIn: "180d" });
    res.json({
      // token: access_token,
      // refresh_token:refresh_token,
      user: userdata,
